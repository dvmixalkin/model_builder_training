...
def set_logger(name=__name__):
    from conveer.utils.LoggerWrapper import SocketHandler
    ...
    set_logging(name)  # run before defining LOGGER
...
def train(hyp, opt, device, tb_writer=None, unmatched_configs=None):
    ...
    with open(opt.data) as f:
        ...
        data_dict = argparse.Namespace(**yaml.load(f, Loader=yaml.SafeLoader))
    data_dict, unmatched_configs = check_opts(data_dict, unmatched_configs, data_path=unmatched_configs['path_to_data'])
...
def parse_args():
    ...
    parser.add_argument('--data', type=str, default='data/scans.yaml', help='data.yaml path')
...
def simple_hyps(opt, hyp, device, unmatched_configs):
...
def evolve_hyps_train(opt, device):
