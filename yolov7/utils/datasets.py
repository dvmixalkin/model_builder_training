    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if hasattr(self, 'pickled_annotation_data'):  # ================================================================
            if self.pickled_annotation_data is not None:  # ============================================================
                from conveer.get_pickle_data import adapt_paths, make_yolo_annotations  # ==============================
                self.label_files = adapt_paths(path, self.label_files, 'annotations')  # ===============================
                make_yolo_annotations(self.img_files, self.label_files, self.pickled_annotation_data)  # ===============
                self.img_files = adapt_paths(path, self.img_files, 'train_target_files')  # ============================
                p = orig_path
        # ==============================================================================================================
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if Path(im_file).suffix == '.npy':
                    im = Image.fromarray(np.load(im_file)).convert('RGB')
                    im.format = 'npy'
                else:
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if Path(path).suffix == '.npy':
            img = np.array(Image.fromarray(np.load(path)).convert('RGB'))
        else:
