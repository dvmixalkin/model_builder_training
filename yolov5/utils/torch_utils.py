...
def model_info(model, verbose=False, img_size=640):
    ...
    LOGGER.debug(f"{name} summary: {len(list(model.modules()))} layers, {n_p} parameters, {n_g} gradients{fs}")
...
class EarlyStopping:
    ...
    def __call__(self, epoch, fitness):
        ...
        if stop:
            LOGGER.debug(f'Stopping training early as no improvement observed in last {self.patience} epochs. '
