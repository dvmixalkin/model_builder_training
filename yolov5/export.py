...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    try:
        ...
        LOGGER.debug(f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        return f
    except Exception as e:
        LOGGER.debug(f'{prefix} export failure: {e}')
    ...
    try:
        ...
        if simplify:
            try:
                ...
                LOGGER.debug(f'{prefix} simplifying with onnx-simplifier {onnxsim.__version__}...')
                ...
                LOGGER.debug(f'{prefix} simplifier failure: {e}')
        LOGGER.debug(f'{prefix} export success, saved as {f} ({file_size(f):.1f} MB)')
        return f
    except Exception as e:
        LOGGER.debug(f'{prefix} export failure: {e}')
...
def export_engine(model, im, file, train, half, simplify, workspace=4, verbose=False, prefix=colorstr('TensorRT:')):
    ...
    try:
        ...
        LOGGER.debug(f'{prefix} Network Description:')
        for inp in inputs:
            LOGGER.debug(f'{prefix}\tinput "{inp.name}" with shape {inp.shape} and dtype {inp.dtype}')
        for out in outputs:
            LOGGER.debug(f'{prefix}\toutput "{out.name}" with shape {out.shape} and dtype {out.dtype}')

        LOGGER.debug(f'{prefix} building FP{16 if builder.platform_has_fast_fp16 and half else 32} engine in {f}')
